...
import '../../data/repositories/unsplash_image_search_repository.dart';
...
@injectable
class UnsplashImageSearchBloc
    with ComponentBuildContext, ComponentLifecycleListener {
  UnsplashImageSearchBloc({required this.imageRepository});

  final UnsplashImageSearchRepository imageRepository;

  Stream<UnsplashImageImageSearchViewModel>? _viewModel;
  Stream<UnsplashImageImageSearchViewModel>? get viewModel => _viewModel;

  final _searchResult =
      BehaviorSubject<UnsplashImageSearchResult?>.seeded(null);

  String? _lastSearchTerm;

  Stream<UnsplashImageImageSearchViewModel> createViewModel() {
    if (_viewModel != null) {
      return _viewModel!;
    }
  ...
  UnsplashImageImageSearchViewModel _createViewModel(
    UnsplashImageSearchResult? searchResult,
  ) {
    return UnsplashImageImageSearchViewModel(
