...
import 'import_google_sheets_bloc.dart';
import 'import_google_sheets_view_model.dart';

class ImportGoogleSheetsComponent extends StatelessWidget {
  const ImportGoogleSheetsComponent({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Component<ImportGoogleSheetsBloc>(
      createViewModel: (bloc) => bloc.createViewModel(),
      builder: (context, bloc) {
        return StreamBuilder<ImportGoogleSheetsViewModel>(
          stream: bloc.viewModel,
          builder: (context, snapshot) {
            final appBarTitle = S.of(context).importFromGoogleSheets;

            switch (snapshot.data?.importState) {
              case ImportState.showInstructions:
                return InstructionsComponent(
                  appBarTitle: appBarTitle,
                  markdownBody: S.of(context).importGoogleSheetsInstructionText,
