...
final _logger = Logger((embedBuilder).toString());

/// Builds a widget for the given [embed].
///
/// Currently, image and divider embeds are supported.
Widget embedBuilder(
...
/// Builds an image for the Quill editor based on the given [url] .
///
/// [url] contains that same string returned from [pickImage].
Widget _buildImage(
  BuildContext context,
  Embed node,
  QuillController controller, {
      ...
      builder: (context, snapshot) {
        ...
        return MouseRegion(
          ...
          child: GestureDetector(
            ...
            onTap:
                readOnly ? null : () => _openImageEditor(imageUrl, controller),
            onLongPress: readOnly ? () => _openAsFullscreen(imageUrl) : null,
            child: ClipRRect(
              borderRadius: BorderRadius.circular(5),
              // Disable the Hero widget until the cards are clickable again and
              // the tag is more specific, so the same image can be displayed
              // on multiple cards. This is important for the offer page.
              // child: Hero(
              // tag: '$imageUrl-hero',
              child: Container(
